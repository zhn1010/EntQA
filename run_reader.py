...
from data_reader import load_data, get_loaders, get_golds, get_results_doc, save_results
...
from run_retriever import configure_optimizer, configure_optimizer_simple, set_seeds
...
def evaluate_rerank(rank_scores, rank_labels):
    ...
    rank_lrap = label_ranking_average_precision_score(ranking_labels, ranking_scores)
    return rank_lrap


def evaluate_after_prune(logger, pruned_preds, golds, samples):
    predicts_doc = get_results_doc(pruned_preds, samples)
    precision, recall, f_1 = compute_strong_micro_results(predicts_doc, golds, logger)
...
def main(args):
    ...
    (
        ...
        p_val_golds,
        p_test_golds,
    ) = get_golds(data[0], data[1], data[2])
    ...
    if args.simpleoptim:
        ...
        optimizer, scheduler, num_train_steps, num_warmup_steps = configure_optimizer(
            args, model, len(data[0])
        )
    ...
    model.to(device)
    loader_train, loader_dev, loader_test = get_loaders(
        tokenizer,
        data,
        args.L,
        args.C,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        val_predicts = transform_predicts(pruned_val_preds, data[-1], data[1])
        val_result = evaluate_after_prune(logger, val_predicts, val_golds_doc, data[1])

        logger.log(
            "Done with epoch {:3d} | train loss {:8.4f} | "
            "val recall  {} | "
            "val precision {} |"
            "val F1 {} |".format(
                epoch,
    ...
    logger.log("test inference time {:s}".format(strtime(start_time_test_infer)))
    logger.log(
        "per val instance inference time {:s}".format(
            str(((datetime.now() - start_time_test_infer) / len(data[2])))
        )
    )
    ...
    save_results(test_predicts, p_test_golds, data[2], args.results_dir, "test")
    test_result = evaluate_after_prune(logger, test_predicts, test_golds_doc, data[2])
    logger.log(
    ...
    pruned_val_preds = prune_predicts(val_raw_predicts, args.thresd)
    val_predicts = transform_predicts(pruned_val_preds, data[-1], data[1])
    logger.log("val inference time {:s}".format(strtime(start_time_val_infer)))
    logger.log(
        "per val instance inference time {:s}".format(
            str(((datetime.now() - start_time_val_infer) / len(data[1])))
        )
    )
    ...
    save_results(val_predicts, p_val_golds, data[1], args.results_dir, "val")
    val_result = evaluate_after_prune(logger, val_predicts, val_golds_doc, data[1])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume_training", action="store_true", help="resume training?"
    )
    parser.add_argument(
        "--no_multi_ents",
        action="store_true",
    ...
    parser.add_argument("--add_topic", action="store_true", help="add title?")
    parser.add_argument(
        "--do_rerank", action="store_true", help="do rerank multi-tasking?"
    )
    parser.add_argument(
        "--stride", type=int, default=16, help="passage stride [%(default)d]"
    )
    parser.add_argument(
        "--max_answer_len",
        type=int,
    ...
    parser.add_argument(
        "--random_positive", action="store_true", help="random positive?"
    )
    parser.add_argument("--oracle", action="store_true", help="oracle evaluation ?")
    parser.add_argument(
    ...
    parser.add_argument(
        "--val_bsz", type=int, default=72, help="batch size [%(default)d]"
    )
    parser.add_argument(
        "--lr", type=float, default=1e-5, help="initial learning rate [%(default)g]"
    )
    parser.add_argument(
        "--warmup_proportion",
        type=float,
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0.01, help="weight decay [%(default)g]"
    )
    parser.add_argument(
        "--adam_epsilon",
        type=float,
    ...
    parser.add_argument(
        "--epochs", type=int, default=3, help="max number of epochs [%(default)d]"
    )
    parser.add_argument(
        "--logging_steps",
        type=int,
    ...
    parser.add_argument(
        "--init", type=float, default=0, help="init (default if 0) [%(default)g]"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="random seed [%(default)d]"
    )
    parser.add_argument(
        "--num_workers", type=int, default=1, help="num workers [%(default)d]"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--simpleoptim",
        action="store_true",
        help="simple optimizer (constant schedule, " "no weight decay?",
    )
    parser.add_argument(
